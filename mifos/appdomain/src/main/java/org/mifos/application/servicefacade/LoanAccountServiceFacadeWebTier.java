...
public class LoanAccountServiceFacadeWebTier implements LoanAccountServiceFacade {
    ...
    public LoanDisbursalDto retrieveLoanDisbursalDetails(Integer loanAccountId) {
        ...
        try {
            ...
            if (backDatedTransactionsAllowed || canOverridePermissionForBackDatedPayments(loan)) {
    ...
    public boolean isTrxnDateValid(Integer loanAccountId, Date trxnDate) {
        ...
        try {
            ...
            if (canOverridePermissionForBackDatedPayments(loan) ||
             loan.isTrxnDateValid(trxnDate, meetingDate, repaymentIndependentOfMeetingEnabled)){
                valueToReturn=true;
            }
...
//        boolean activeSurveys = surveysPersistence.isActiveSurveysForSurveyType(SurveyType.LOAN);
    ...
    public boolean canOverridePermissionForBackDatedPayments (LoanBO loanBo) {
        loanBo.setUserContext(getUserContext());
        AccountPaymentEntity lastPmntToBeAdjusted = loanBo.getLastPmntToBeAdjusted();
        Date lastPaymentDate;
        if (lastPmntToBeAdjusted == null) {
            lastPaymentDate = loanBo.getDisbursementDate();
        }
            else
            {
                lastPaymentDate = lastPmntToBeAdjusted.getPaymentDate();
            }
        UserContext userContext = loanBo.getUserContext();
        PersonnelBO personnel = loanBo.getPersonnel();
        Short personnelId = personnel != null? personnel.getPersonnelId() : userContext.getId();
        Short officeId = loanBo.getOfficeId();
        return ActivityMapper.getInstance().isAdjustmentPermittedForBackDatedPayments(lastPaymentDate, userContext,officeId,personnelId);
    }
